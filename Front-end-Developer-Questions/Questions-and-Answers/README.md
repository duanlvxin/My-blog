...
- rem布局的优缺点
		rem的优点：能让我们在手机各个机型的适配方面；大大减少我们代码的重复性，是我们的代码更兼容。
		rem的缺点：目前ie不支持 对pc页面来讲使用次数不多；数据量大：所有的图片，盒子都需要我们去给一个准确的值；才能保证不同机型的适配；

-@media的优缺点
		@media的优点：应用广泛  适用于 pc端  手机页面，通常做自适应布局时  我们比较常用。
		@media的缺点：缺点：相对于代码要重复很多 ； 可能存在闪屏的问题出现
...
-  js中==是如何比较的？
		...
		1.如果有一个操作数是布尔值，则在比较相等性之前先将其转为数值，false转为0，true转为1
		
		2.如果有一个操作数是字符串，则在比较相等性之前先将其转为数值,使用Number()方法；
		
		3.如果一个操作时是对象，另一个不是，则调用对象的ValueOf()方法，用得到的基本类型值按照前面的规则进行比较；
		
		4.同时，需要遵循以下规则：（1）null和undefined是相等的；（2）在比较相等性之前，不能将null和undefined转换为其他任何值
		 （3）如果至少有一个操作数是NaN，相等操作符返回false，不相等操作符返回true
		 （4）如果两个操作数都是对象，则比较它们是不是同一个对象。如果两个操作数指向同一个对象，则相等操作数返回true，否则，返回false
